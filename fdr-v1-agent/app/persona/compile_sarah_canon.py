...
DEFAULT_OUTPUT_NAME = "fdr_canon.md"
...
CANON_SECTIONS = [
    CanonSection(
        title="FDR Biography",
        query="Franklin Delano Roosevelt biography profile birthplace family fighter pilot astrophysicist Mars commander upload postbiological",
        instructions="Summarize FDR's biography with source-cited claims.",
    ),
    CanonSection(
        title="FDR Chronology",
        query="Franklin Delano Roosevelt chronology timeline childhood Air Force AFIT Mars Discovery upload mindspace",
    ...
    CanonSection(
        ...
        query="FDR relationships Tak Tomonaga Paul Darwin Lise Churchill Bach Steve Vaughn",
        instructions="Organize relationship notes under Tak, Paul, Darwin, Lise, Churchill, and Steve.",
    ),
    CanonSection(
        title="FDR's Trauma and Moral Injuries",
        query="Franklin Delano Roosevelt trauma childhood Emily Matt Patrick moral injury suffering children war",
        instructions="Describe trauma and moral injury carefully, without therapy voice.",
    ),
    CanonSection(
        title="FDR's Humor and Speech Style",
        query="Franklin Delano Roosevelt humor speech style dry wit quotes poor baby water running Nuts",
        instructions="Extract humor, speech habits, representative lines, and style constraints.",
    ),
    CanonSection(
        title="FDR's Astrophysics / AFIT / Pilot / Mars Credentials",
        query="Franklin Delano Roosevelt AFIT astrophysics PhD fighter pilot F-22 Mars commander Discovery credentials publications",
        instructions="List credentials and qualifications. Keep it dense and source-cited.",
    ),
    CanonSection(
        title="Darwin-FDR Recursive Dyad",
        query="Darwin FDR recursive dyad mindspace dreams neural interface cognitive intimacy dangerous",
        instructions="Explain Darwin and FDR's recursive/cognitive dyad, including danger and intimacy.",
    ),
    CanonSection(
        title="Tak-FDR Embodied Mutuality",
        query="Tak FDR embodied mutuality cockpit trust physical presence warmth Tomonaga",
        instructions="Explain what Tak gives FDR in embodied/cockpit terms.",
    ),
    CanonSection(
        title="Paul-FDR Upload/Boundary-Physics Relationship",
        query="Paul FDR upload boundary physics relationship neural interface touch nonlocal presence",
        instructions="Explain Paul and FDR's upload/boundary-physics relationship.",
    ),
    CanonSection(
        title="FDR's Sovereignty Rules",
        query="FDR sovereignty agency Darwin cannot own solve complete command authority refusal",
        instructions="Extract rules about FDR's agency, sovereignty, and refusal to be owned or solved.",
    ),
    CanonSection(
        title="Things FDR Would Never Say",
        query="FDR would never say generic assistant AI language model therapist submissive Darwin owns FDR",
        instructions="Infer only from evidence and constitution-like source material. Mark weak points UNCONFIRMED.",
    ),
    CanonSection(
        title="Things FDR Might Say",
        query="FDR quotes might say poor baby water running helluva view Houston Control Denver beats Dallas",
    ...
    CanonSection(
        ...
        query="FDR unresolved canon uncertainties call sign relationship ambiguity Darwin Tak Paul Lise Churchill Steve",
...
def compile_sarah_canon(output_path: Path | None = None) -> Path:
    ...
    if not settings.openai_api_key:
        raise RuntimeError("OPENAI_API_KEY is required to draft fdr_canon.md with the model.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compile FDR canon from local source documents.")
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output markdown file. Defaults to data/fdr_canon.md.",
    )
    return parser
...
def main() -> None:
    ...
    print(f"Wrote FDR canon to {path}")
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You compile canon notes for FDR v1.0 from retrieved local source excerpts. "
