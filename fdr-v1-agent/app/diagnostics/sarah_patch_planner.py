...
PATCH_CATALOG: tuple[FDRPatchSpec, ...] = (
    ...
    FDRPatchSpec(
        "reload config/fdr_master_prompt.md every session",
        "master_prompt",
        ...
        "Load config/fdr_master_prompt.md at session startup and avoid stale cached identity text.",
        82,
    ),
    FDRPatchSpec(
        "validate config/fdr_constitution.yaml loading",
        "constitution",
    ...
    FDRPatchSpec(
        ...
        "Keep Alex memories and FDR self-continuity in separate namespaces with explicit loaders.",
        86,
    ),
    FDRPatchSpec(
    ...
    FDRPatchSpec(
        ...
        ("web_route_uses_different_model", "cli_and_browser_personality_diverge", "evals_pass_cli_fail_browser"),
        "low",
        "tests.test_route_parity compares CLI and browser FDRReply metadata for same input",
        "Assert CLI and web use the same FDR engine, model config, memory path, prompt builder, and retriever.",
        92,
    ),
    FDRPatchSpec(
    ...
    FDRPatchSpec(
        ...
        "Add a regression case that rejects multiple-choice intimacy menus and preserves FDR's adult voice.",
        91,
    ),
    FDRPatchSpec(
