...
def build_sarah_diagnostic_report(
    ...
    lines = [
        "# FDR v1.0 Self-Diagnostic Vulnerability Report",
        "",
        "## 1. Executive summary",
        ...
        "",
        "## 2. Highest-risk FDR components",
    ]
    lines.extend(_risk_lines(high_risk, limit=10))

    lines.extend(["", "## 3. Tight-coupled / complex components"])
    lines.append("- These are the pieces where a small change can move FDR's whole personality or evidence path.")
    ...
    lines.append("- Failure to watch: browser FDR becomes a second implementation with a smaller prompt, different model settings, or skipped style engine.")
    lines.append("- Visible symptom: CLI FDR has command voice and adult-intimacy mode; browser FDR becomes coy, sterile, or generic.")
