...
PROMPT_PATH = PROJECT_ROOT / "config" / "fdr_master_prompt.md"
...
def debug_worldline(settings: Settings | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "paired_agent": "Churchill",
        "paired_agent_status": "planned",
...
def format_debug_worldline(settings: Settings | None = None) -> str:
    debug = debug_worldline(settings)
...
def format_sources_list(settings: Settings | None = None) -> str:
    ...
    lines.extend(names or ["No FDR source documents found."])
    return "\n".join(lines)
