...
def load_settings(env_file: Path = ENV_FILE) -> Settings:
    ...
    return Settings(
        ...
        sarah_model=env.get("AGENT_MODEL") or env.get("FDR_MODEL") or env.get("SARAH_MODEL", "gpt-5.2"),
        ...
        external_source_docs_dir=PROJECT_ROOT / "data" / "source_docs",
        vector_store_dir=PROJECT_ROOT / "data" / "vector_store",
        conversations_dir=PROJECT_ROOT / "data" / "conversations",
        web_cache_dir=PROJECT_ROOT / "data" / "web_cache",
        memory_file=PROJECT_ROOT / "data" / "memory" / "fdr_memory.jsonl",
