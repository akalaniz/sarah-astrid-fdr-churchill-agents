...
from app.orchestration.agent_orchestrator import format_debug_orchestrator, is_agent_endpoint_reachable, run_multi_agent_dialogue
...
def run_chat_loop(settings: Settings) -> None:
    logger.info("Starting FDR chat loop with model=%s", settings.sarah_model)
    ...
    while True:
        ...
        if command == "/reset":
            ...
            print("FDR: Active conversation history cleared.")
            continue
        ...
        if command.startswith("/crew "):
            ...
            if not topic:
                ...
                continue
            if not is_agent_endpoint_reachable("Churchill"):
                print(_churchill_not_running_message())
                continue
            try:
                result = run_multi_agent_dialogue(topic=topic, agents=["FDR", "Churchill"], rounds=4)
        ...
        if command.startswith("/send "):
            try:
                ...
                continue
            print(f"FDR: Sent to {message['to_agent']} as {message['id']} in thread {message['conversation_id']}.")
            continue
        ...
        if command == "/archive_inbox":
            print(f"FDR: Archived {archive_inbox(LOCAL_AGENT_NAME)} messages addressed to {LOCAL_AGENT_NAME}.")
            continue

        if command == "/clear_inbox_confirm":
            print(f"FDR: Cleared {clear_inbox(LOCAL_AGENT_NAME)} messages addressed to {LOCAL_AGENT_NAME}.")
            continue
        ...
        if command.startswith("/reply "):
            try:
                ...
                continue
            print(f"FDR: Replied to {message['to_agent']} in thread {message['conversation_id']} as {message['id']}.")
            continue
        ...
        if command.startswith("/remember "):
            ...
            print(f"FDR: I’ll remember that. ({record.memory_type})")
            continue
        ...
        if command.startswith("/forget "):
            ...
            print(f"FDR: Forgot {len(forgotten)} matching memor{'y' if len(forgotten) == 1 else 'ies'}.")
            continue
        ...
        try:
            ...
            print(f"FDR: OpenAI request failed: {exc}")
            continue
...
def _parse_send_command(user_input: str) -> tuple[str, str]:
    ...
    if ":" not in payload:
        raise ValueError('Use /send Churchill: message text')
...
def _handle_web_cache_command(user_input: str) -> str | None:
    ...
    try:
        ...
        if command == "/web_cache_clear":
            return "This will delete all FDR web-cache JSON files from data\\web_cache. To confirm, run: /web_cache_clear confirm"
        ...
        if command == "/web_cache_archive_all":
            return "To archive all FDR web-cache JSON files, run: /web_cache_archive_all confirm"
        ...
        if command == "/web_cache_delete_corrupt":
            return "This will delete corrupt FDR web-cache JSON files only. To confirm, run: /web_cache_delete_corrupt confirm"
