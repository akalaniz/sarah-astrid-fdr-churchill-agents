...
def generate_sarah_reply(user_message: str, session_id: str = "default") -> FDRReply:
    ...
    with _lock:
        ...
        memory_context_override, injected_count, recall_source_session_id = _prepare_memory_context(
            settings,
            session,
            message,
            session_id,
...
def remember_sarah_memory(text: str, session_id: str = "default") -> MemoryRecord:
    settings = _get_settings()
    return MemoryStore(settings.memory_file).remember(text)


def list_sarah_memories(keyword: str = "") -> list[MemoryRecord]:
    settings = _get_settings()
    store = MemoryStore(settings.memory_file)
    return store.search(keyword) if keyword.strip() else store.list_memories()


def format_sarah_memories(keyword: str = "") -> str:
    return format_memories(list_sarah_memories(keyword))


def recall_sarah_memories(keyword: str, session_id: str = "default") -> str:
    global _shared_pending_recall
    settings = _get_settings()
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    ...
    if not memories:
        return f"No FDR memories matched '{keyword}'."
    lines = [f"Recalled FDR memories for '{keyword}':"]
    lines.extend(f"- {memory.memory_type}: {memory.text}" for memory in memories[:8])
    return "\n".join(lines)


def recall_all_sarah_memories(session_id: str = "default") -> str:
    global _shared_pending_recall
    settings = _get_settings()
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    ...
    _shared_pending_recall = (session_id, session.last_recall_keyword, memories) if memories else None
    if not memories:
        return "No FDR memories are stored."
    lines = ["Loaded all memories. This may make the next response less focused."]
...
def set_sarah_use_memory(enabled: bool, session_id: str = "default") -> str:
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    session.use_memory = enabled
    return f"use_memory: {'on' if enabled else 'off'}"


def clear_sarah_recall(session_id: str = "default") -> str:
    global _shared_pending_recall
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    session.pending_recall = []
    session.last_recall_keyword = ""
    if _shared_pending_recall and _shared_pending_recall[0] == session_id:
        _shared_pending_recall = None
    return "Pending recalled FDR memory context cleared."


def debug_sarah_memory_live(session_id: str = "default") -> dict[str, Any]:
    settings = _get_settings()
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    store = MemoryStore(settings.memory_file)
...
def format_debug_sarah_memory_live(session_id: str = "default") -> str:
    debug = debug_sarah_memory_live(session_id)
...
def _prompt_had_memory(response: FDRResponse) -> bool:
    ...
    return "Relevant remembered context from FDR memory:" in block and "No relevant FDR memories injected" not in block
...
def _extract_memory_block(response: FDRResponse) -> str:
    for message in response.messages:
        if "Relevant remembered context from FDR memory:" in message.get("content", ""):
            return message.get("content", "")
    return ""


def _forbidden_memory_paths_detected(memory_file) -> bool:
    path = str(memory_file).lower()
    forbidden = (
