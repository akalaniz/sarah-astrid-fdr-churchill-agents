...
class ChatCoreTests(unittest.TestCase):
    ...
    def test_default_memory_file_is_astrid_memory(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            with tempfile.TemporaryDirectory() as tmp:
                settings = load_settings(env_file=Path(tmp) / ".env")

        self.assertEqual(settings.memory_file.name, "astrid_memory.jsonl")
        self.assertIn("data", settings.memory_file.parts)
        self.assertIn("memory", settings.memory_file.parts)

    def test_default_source_docs_and_vector_store_are_astrid_local_paths(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            with tempfile.TemporaryDirectory() as tmp:
                settings = load_settings(env_file=Path(tmp) / ".env")

        self.assertEqual(settings.source_docs_dir, settings.project_root / "data" / "source_docs")
        self.assertEqual(settings.external_source_docs_dir, settings.source_docs_dir)
        self.assertEqual(settings.vector_store_dir, settings.project_root / "data" / "vector_store")

    def test_agent_memory_file_overrides_sarah_fallbacks(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            with tempfile.TemporaryDirectory() as tmp:
                root = Path(tmp)
                env_file = root / ".env"
                env_file.write_text(
                    "\n".join(
                        [
                            "AGENT_MEMORY_FILE=custom/astrid_memory.jsonl",
                            "SARAH_MEMORY_FILE=data/memory/sarah_memory.jsonl",
                            "SARAH_MEMORY=data/memory/also_sarah_memory.jsonl",
                        ]
                    ),
                    encoding="utf-8",
                )
        ...
        self.assertEqual(settings.memory_file.name, "astrid_memory.jsonl")
        self.assertIn("custom", settings.memory_file.parts)

    def test_existing_default_astrid_memory_blocks_legacy_sarah_memory_env(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            with tempfile.TemporaryDirectory() as tmp:
                root = Path(tmp)
                default_memory = root / "data" / "memory" / "astrid_memory.jsonl"
                default_memory.parent.mkdir(parents=True)
                default_memory.touch()
                env_file = root / ".env"
                env_file.write_text("SARAH_MEMORY_FILE=data/memory/legacy_sarah_memory.jsonl\n", encoding="utf-8")
                with patch("app.core.config.PROJECT_ROOT", root):
                    settings = load_settings(env_file=env_file)

        self.assertEqual(settings.memory_file, default_memory)

    def test_legacy_sarah_memory_env_is_ignored_when_astrid_memory_is_absent(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            with tempfile.TemporaryDirectory() as tmp:
                root = Path(tmp)
                env_file = root / ".env"
        ...
        self.assertEqual(settings.memory_file, root / "data" / "memory" / "astrid_memory.jsonl")
