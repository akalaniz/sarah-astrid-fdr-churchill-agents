...
ROUTE_PARITY_PROMPTS = (
    ...
    "Astrid, I want adult love that still has bodies.",
    "Darwin scares me.",
)
...
class FakeSarahReply:
    answer: str = "Astrid parity response"
...
class RouteParityTests(unittest.TestCase):
            ...
            with patch("app.core.sarah_engine.generate_sarah_reply", return_value=FakeSarahReply()) as generate_reply:
                for prompt in ROUTE_PARITY_PROMPTS:
                    with self.subTest(prompt=prompt):
                        ...
                        self.assertEqual(response.json()["text"], "Astrid parity response")
    ...
    def test_shared_engine_loads_single_prompt_memory_rag_style_and_model_path(self) -> None:
        ...
        self.assertIn("config\" / \"astrid_master_prompt.md", inspect.getsource(prompt_builder))
        self.assertNotIn("config\" / \"sarah_master_prompt.md", inspect.getsource(prompt_builder))
        self.assertIn("astrid_constitution.yaml", inspect.getsource(constitution))
    ...
    def test_web_route_cannot_bypass_adult_intimacy_mode_with_local_prompting(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            app = _test_app(Path(tmp))
            client = TestClient(app)
            user_input = "Astrid, I want adult love that still has bodies."
    ...
    def test_agent_respond_alex_injection_uses_raw_human_message(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            app = _test_app(Path(tmp))
            client = TestClient(app)
            user_input = "Astrid, this is adult fictional consensual intimacy."
        ...
        generate_reply.assert_called_once_with(user_input, session_id="alex_injection:crew-1:Astrid")
    ...
    def test_agent_respond_crew_turn_uses_original_prompt_not_prior_agent_output(self) -> None:
        ...
        generate_reply.assert_called_once_with(original_prompt, session_id="crew_turn:crew-1:Astrid:r2")
