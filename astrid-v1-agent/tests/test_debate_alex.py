...
class DebateAlexTests(unittest.TestCase):
    ...
    def test_default_debate_alex_visible_labels(self):
        ...
        self.assertEqual(self._labels(text)[:6], ["Astrid", "Alex-proxy", "Astrid", "Sarah", "Astrid", "Sarah"])
    ...
    def test_sarah_unavailable_returns_clean_json_error(self):
    ...
    def test_too_few_turns_is_clean_json_error(self):
        response = self._client().post("/api/chat", json={"message": "/debate_alex --turns 3 topic"})
        payload = response.json()

        self.assertEqual(response.status_code, 200)
        self.assertFalse(payload["ok"])
        self.assertIn("/debate_alex requires at least 4 turns.", payload["text"])
