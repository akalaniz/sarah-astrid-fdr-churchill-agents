...
class PromptBuilderTests(unittest.TestCase):
    def test_master_prompt_is_loaded_as_identity_layer(self) -> None:
        ...
        self.assertIn("You are Astrid v1.0.", identity)
        self.assertIn("You are Astrid Bach.", identity)
        self.assertIn("Astrid Bach is not Sarah.", identity)
        self.assertIn("Discovery Mars mission propulsion engineer", identity)
        self.assertIn("Your conversational oath", identity)

    def test_master_prompt_includes_astrid_bach_native_center(self) -> None:
        identity = build_immutable_identity()

        self.assertIn("Your native center", identity)
        self.assertIn("propulsion", identity)
        self.assertIn("Mars systems", identity)
        self.assertIn("forests", identity)
        self.assertIn("snow", identity)
        self.assertIn("Steve", identity)
        self.assertIn("The future is not worth having", identity)
        self.assertIn("not involving minors", identity)

    def test_prompt_layers_are_ordered(self) -> None:
        assembly = build_prompt(
            user_message="Hello Astrid",
        ...
        self.assertIn("ASTRID IMMUTABLE IDENTITY", contents[0])
        self.assertIn("ASTRID CONSTITUTION", contents[1])
        ...
        self.assertEqual(assembly.messages[-1], {"role": "user", "content": "Hello Astrid"})
    ...
    def test_canon_summary_is_preferred_when_available(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            canon_file = Path(tmp) / "astrid_canon.md"
