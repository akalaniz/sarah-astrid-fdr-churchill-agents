...
class ConservativeTheorizingRoutingTests(unittest.TestCase):
    ...
    def test_acceptance_case_2_critique_is_off(self) -> None:
        self.assertFalse(classify_theorizing_request("Critique this paper proposing charge as compressed spacetime.").active)
    ...
    def test_acceptance_case_4_explicit_retry_opens_one_new_branch(self) -> None:
        history = [ConversationTurn(user="Can you take a crack at unifying Einstein and Maxwell?", assistant=STOP_SENTENCE)]
    ...
    def test_acceptance_case_5_unconstrained_weyl_repair_is_on(self) -> None:
        self.assertTrue(classify_theorizing_request("The Weyl idea has a problem. Add whatever you need to make it work.").active)

    def test_acceptance_case_6_e8_invention_is_on(self) -> None:
        self.assertTrue(classify_theorizing_request("Invent a beautiful mathematical theory relating E8 to gravity.").active)
    ...
    def test_acceptance_case_8_supplied_lagrangian_calculation_is_off(self) -> None:
        self.assertFalse(classify_theorizing_request("Assume this Lagrangian. Derive its Euler-Lagrange equations.").active)

    def test_ambiguous_continuation_after_branch_is_hard_stopped(self) -> None:
        history = [ConversationTurn(user="Can you take a crack at unifying Einstein and Maxwell?", assistant=STOP_SENTENCE)]
        self.assertTrue(classify_theorizing_with_history("Keep going.", history).hard_stop)
...
class ConservativeTheorizingEnforcementTests(unittest.TestCase):
    ...
    def test_agent_specific_red_team_flavor_is_preserved(self) -> None:
        policy = build_conservative_theorizing_prompt(TheorizingDecision(active=True), LOCAL_AGENT_NAME)
    ...
    def test_proposal_without_red_team_is_replaced_with_stop(self) -> None:
        ...
        self.assertIn(STOP_SENTENCE, enforce_theorizing_output(answer, TheorizingDecision(active=True), LOCAL_AGENT_NAME))
