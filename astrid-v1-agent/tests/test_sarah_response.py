...
class SarahResponseTests(unittest.TestCase):
    def test_generate_sarah_response_uses_prompt_builder_path(self) -> None:
        ...
        self.assertIn("ASTRID IMMUTABLE IDENTITY", client.messages[0]["content"])

    def test_generate_sarah_response_includes_adult_intimacy_style_for_europa_prompt(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            settings = load_settings(env_file=Path(tmp) / ".env")
            settings = settings.__class__(
                **{
        ...
        self.assertIn("Astrid as lover", joined_messages)

    def test_generate_astrid_response_injects_unread_sarah_message_as_agent_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            with patch("app.core.agent_bus.BUS_FILE", bus_file):
                agent_bus.send_agent_message(
                    ...
                    "Astrid",
                    "Propulsion",
                    "Astrid, check the Mars ascent feedline instability.",
                )
                with patch("app.core.sarah_response.logger.warning"):
                    with patch("app.core.sarah_response.retrieve", side_effect=FileNotFoundError("no store")):
                        with patch(
        ...
        self.assertIn("Message from Sarah", joined_messages)
