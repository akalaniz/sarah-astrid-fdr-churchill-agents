...
function createPdfPrintRoot(mode) {
  ...
  if (mode === "conversation") {
    ...
    const responses = messagesEl.querySelectorAll('.message.astrid[data-pdf-response="true"]');
    const lastResponse = responses[responses.length - 1];
    if (!lastResponse) {
      window.alert("No Astrid response is available to export yet.");
      return null;
    }
...
async function exportConversationPdf() {
  ...
  document.title = `Astrid-conversation-${new Date().toISOString().slice(0, 10)}`;
  try {
    window.print();
  } catch (error) {
    cleanupPdfPrint();
...
async function exportLastResponsePdf() {
  ...
  if (!root) return;
  originalPdfPrintTitle = document.title;
  document.title = `Astrid-last-response-${new Date().toISOString().slice(0, 10)}`;
  try {
    window.print();
  } catch (error) {
    cleanupPdfPrint();
...
async function ingestPdf(file) {
  ...
  try {
    ...
    pdfIngestStatus.textContent = payload.indexed
      ...
      : `${payload.filename} is already in Astrid's sources.`;
  } catch (error) {
...
function addMessage(role, text) {
  ...
  label.textContent = role === "user" ? "You" : "Astrid";
...
async function loadAgentInbox(showAll = false) {
  try {
    ...
    if (!payload.messages.length) {
      agentInboxPanel.textContent = showAll ? "No inter-agent messages for Astrid." : "No unread direct/reply inter-agent messages.";
      return;
    }
...
async function archiveAgentInbox() {
  ...
  addMessage("astrid", `Archived ${payload.archived} messages addressed to Astrid.`);
  loadAgentInbox();
}

async function clearAgentInbox() {
  const confirmed = window.confirm("Clear only messages addressed to Astrid from the shared bus?");
  if (!confirmed) return;
  ...
  addMessage("astrid", `Cleared ${payload.cleared} messages addressed to Astrid.`);
  loadAgentInbox();
}
...
async function sendAgentMessage() {
  ...
  try {
    const response = await fetch("/agent/send", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ to_agent: "Sarah", subject: "Message from Astrid", body }),
    });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.detail || "Agent send failed.");
    agentMessageInput.value = "";
    addMessage("astrid", `Sent to Sarah as ${payload.message.id}.`);
  } catch (error) {
    addMessage("astrid", `Agent send failed: ${error.message}`);
  } finally {
...
async function resetConversation() {
  ...
  addMessage("astrid", "Active conversation reset.");
}
...
function setupMicrophone() {
  ...
  recognition.onerror = (event) => {
    addMessage("astrid", `Microphone transcription failed: ${event.error}`);
...
function setupSpeechSynthesis() {
  ...
  const refreshVoices = () => {
    ...
    selectedVoice = chooseAstridVoice(voices);
...
function chooseAstridVoice(voices) {
...
function findPreferredVoice(voices) {
  return voices.find((voice) => {
    ...
    return PREFERRED_ASTRID_VOICES.some((pattern) => pattern.test(label));
  });
}
...
function voicePreferenceScore(voice) {
  ...
  if (PREFERRED_ASTRID_VOICES.some((pattern) => pattern.test(label))) score += 80;
