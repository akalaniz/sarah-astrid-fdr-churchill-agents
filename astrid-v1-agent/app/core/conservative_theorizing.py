...
_CREATIVE_BYPASS = (
    "science fiction", "sci-fi", "for my novel", "for a novel", "for my story", "worldbuilding", "fictional theory",
)
_CRITIQUE_BYPASS = (
    "critique", "red-team", "red team", "review this paper", "review the evidence", "check these equations",
    "check this equation", "historical discussion",
)
_CALCULATION_BYPASS = (
    "assume this lagrangian", "given this lagrangian", "derive its euler-lagrange", "derive the euler-lagrange",
    "calculate the consequences", "compute the consequences",
)
_GENERATIVE_ACTIONS = (
    "invent", "propose", "construct", "formulate", "develop a theory", "new theory", "new lagrangian",
    "new action", "take a crack", "unify", "unifying", "unification", "modify", "extend the theory",
    "speculate", "make it work", "add whatever",
)
_FUNDAMENTAL_SUBJECTS = (
    "fundamental physics", "general relativity", "einstein", "maxwell", "electromagnetism", "quantum mechanics",
    "quantum gravity", "qft", "standard model", "lagrangian", "gauge field", "new field", "new force",
    "new symmetry", "extra dimension", "new dimension", "geometry", "topology", "new particle", "e8", "weyl",
    "dark matter", "dark energy", "unexplained physical",
)
_EXPLICIT_RETRIES = (
    "try another approach", "try a different approach", "try again", "another speculative branch", "keep speculating",
    "continue speculating", "give it another shot",
    "extend that theory", "extend the proposal", "build on that theory",
)
_UNAUTHORIZED_CONTINUATIONS = ("continue", "go on", "keep going", "what next", "fix it", "add more", "repair it")
...
def classify_theorizing_request(user_message: str, previous_theorizing: bool = False) -> TheorizingDecision:
    ...
    if previous_theorizing and _is_short_continuation(text):
        return TheorizingDecision(True, hard_stop=True, reason="previous branch is closed and no explicit retry was requested")
...
def classify_theorizing_with_history(user_message: str, history: Iterable[Any]) -> TheorizingDecision:
    previous = False
    for turn in list(history)[-6:]:
        decision = classify_theorizing_request(getattr(turn, "user", ""), previous_theorizing=previous)
...
def build_conservative_theorizing_prompt(decision: TheorizingDecision, agent_name: str) -> str:
    ...
    if agent_name.lower().startswith("sarah"):
        flavor = (
            "SARAH-SPECIFIC RED TEAM\nBias the audit toward observable consequences, experimental kill tests, "
            "numerical changes, survival against known data, and correct gauge symmetries and physical degrees of freedom. "
            "Your recurring question is: What would we measure that tells us this is more than a story?"
        )
    else:
        flavor = (
            "ASTRID-SPECIFIC RED TEAM\nBias the audit toward degree-of-freedom proliferation, arbitrary parameters or "
            "functions, ansatze that bake in the result, unnecessary machinery, stability, consistency, and whether "
            "explanatory compression increased. Your recurring question is: What did this buy us, and what did we pay for it?"
        )
...
def enforce_theorizing_output(answer: str, decision: TheorizingDecision, agent_name: str) -> str:
    ...
    if "proposal:" not in lowered:
        allowed_no_proposal = (
            STOP_SENTENCE.lower() in lowered or "no new theory" in lowered or "already answers" in lowered or "already resolves" in lowered
        )
    ...
    if _declares_replacement(replacement_check):
        ...
        return answer
    progress_markers = (
        "falsifiable prediction", "measurable prediction", "new constraint", "forced relation", "no-go",
        "empirical discriminator", "reduces the number", "fewer free parameters", "experimental kill test",
    )
...
def crew_theorizing_control(original_prompt: str, from_agent: str, prior_message: str) -> str:
    ...
    lines = [f"CONSERVATIVE_THEORIZING_ROLE: {role}", "This is one conservative-theorizing branch."]
    if role == "skeptic":
        lines.extend(
            [
