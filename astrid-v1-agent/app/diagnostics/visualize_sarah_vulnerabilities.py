...
def _perrow_matrix_html(graph: nx.DiGraph) -> str:
    ...
    svg = f"""
    <svg viewBox="0 0 {width} {height}" role="img" aria-label="Astrid Perrow matrix">
      <rect width="{width}" height="{height}" fill="#fbfcff"/>
      <text x="{width / 2}" y="32" text-anchor="middle" class="title">Astrid Perrow Matrix</text>
      {grid}
    ...
    """
    return _html_page("Astrid Perrow Matrix", svg, _legend_html())
...
def _cascade_timeline_html(cascade: dict[str, Any]) -> str:
    ...
    svg = f"""
    <svg viewBox="0 0 {width} {height}" role="img" aria-label="Sarah cascade timeline">
      <rect width="{width}" height="{height}" fill="#fbfcff"/>
    ...
    return _html_page("Astrid Cascade Timeline", svg, legend)
...
def _patch_priority_html(patches: list[dict[str, Any]]) -> str:
    ...
    svg = f"""
    <svg viewBox="0 0 {width} {height}" role="img" aria-label="Sarah patch priorities">
      <rect width="{width}" height="{height}" fill="#fbfcff"/>
    ...
    """
    return _html_page("Astrid Patch Priorities", svg, "<p>Ranked by expected risk reduction. Priority also includes centrality, known findings, cascade observations, and difficulty.</p>")
...
def _route_divergence_html(graph: nx.DiGraph) -> str:
    ...
    nodes = {
        ...
        "engine": (360, 235, "Shared Astrid engine", "#57a773"),
    ...
    svg = f"""
    <svg viewBox="0 0 {width} {height}" role="img" aria-label="Sarah route divergence graph">
      <rect width="{width}" height="{height}" fill="#fbfcff"/>
    ...
    """
    return _html_page("Astrid Route Divergence", svg, diff_notes)
...
def _legend_html() -> str:
    return (
        "<p>Read this like Perrow for Sarah herself: upper-right points are tightly coupled, highly complex pieces of the construct. "
...
def _graphml_safe_copy(graph: nx.DiGraph) -> nx.DiGraph:
    safe = nx.DiGraph(name=graph.graph.get("name", "Astrid vulnerability graph"))
    for node_id, attrs in graph.nodes(data=True):
