...
REPRESENTATIVE_CASCADES: tuple[str, ...] = (
    ...
    "Astrid doc not retrieved for intimacy",
...
START_ALIASES: dict[str, str] = {
    ...
    "astrid doc not retrieved for intimacy": "astrid_doc_not_retrieved_for_intimacy",
    "astrid_doc_not_retrieved_for_intimacy": "astrid_doc_not_retrieved_for_intimacy",
...
def resolve_start_failure(start: str) -> str:
    ...
    raise ValueError(f"Unknown Sarah cascade start: {start}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Astrid v1.0 self-diagnostic vulnerability graph analysis.")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--json", action="store_true", help="Print machine-readable JSON output.")

    subparsers = parser.add_subparsers(dest="command")
    subparsers.add_parser("build", help="Build and export the Astrid vulnerability graph.")
    subparsers.add_parser("score", help="Score the Astrid vulnerability graph.")

    simulate_parser = subparsers.add_parser("simulate", help="Run a time-stepped Sarah cascade simulation.")
    ...
    subparsers.add_parser("patch", help="Recommend Sarah patches from graph and latest cascade.")
...
def main() -> None:
    ...
    if command == "build":
        ...
        _print_paths("Sarah graph build complete.", paths, as_json=args.json)
        return
    ...
    if command == "score":
        ...
        if args.json:
            ...
            print("Sarah graph score complete.")
    ...
    if command == "simulate":
        ...
        if args.json:
            ...
            print("Sarah cascade simulation complete.")
    ...
    if command == "patch":
        ...
        if args.json:
            ...
            print("Sarah patch plan complete.")
            print(f"patch_plan: {patch_path}")
        return

    if command == "full-run":
        paths = run_full(output_dir=args.output_dir)
        _print_paths("Sarah diagnostics complete.", paths, as_json=args.json)
        return
