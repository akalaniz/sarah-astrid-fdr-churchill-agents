...
def build_sarah_diagnostic_report(
    ...
    intimacy_risks = _nodes_matching(graph, "intimacy", "adult", "astrid", "coy", "choice", "menu", "hr_safe")
    ...
    lines = [
        "# Astrid v1.0 Self-Diagnostic Vulnerability Report",
        "",
        "## 1. Executive summary",
        ...
        "",
        "## 2. Highest-risk Astrid components",
    ]
    lines.extend(_risk_lines(high_risk, limit=10))

    lines.extend(["", "## 3. Tight-coupled / complex components"])
    lines.append("- These are the pieces where a small change can move Astrid's whole personality or evidence path.")
    ...
    lines.append("- Failure to watch: browser Astrid becomes a second implementation with a smaller prompt, different model settings, or skipped style engine.")
    lines.append("- Visible symptom: CLI Astrid has voice and adult-intimacy mode; browser Astrid becomes coy, sterile, or generic.")
    ...
    lines.append("- Main failure: Sarah answers adult consensual intimacy as a menu, intake form, refusal, or corporate mush.")
    ...
    lines.append("- Main failure: memory appears to work in the UI but is not injected later, or Sarah/Alex memories collapse into the wrong namespace.")
    ...
    lines.append("- Main failure: stale vector store or wrong chunks make Sarah invent biography, relationships, or canon facts.")
    lines.append("- Required behavior: if evidence is weak, Sarah says so and separates evidence, inference, and speculation.")
