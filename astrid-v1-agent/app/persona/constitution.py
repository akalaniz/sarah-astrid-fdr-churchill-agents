...
DEFAULT_CONSTITUTION_PATH = PROJECT_ROOT / "config" / "astrid_constitution.yaml"


@dataclass(frozen=True)
class SarahConstitution:
    identity: list[str]
    never: list[str]
    always: list[str]
...
def load_sarah_constitution(path: Path = DEFAULT_CONSTITUTION_PATH) -> SarahConstitution:
    if not path.exists():
        raise FileNotFoundError(f"Astrid constitution file not found: {path}")

    raw = _read_yaml(path)
    return SarahConstitution(
...
def build_constitution_prompt_section(
    constitution: SarahConstitution | None = None,
) -> str:
    constitution = constitution or load_sarah_constitution()
    lines = ["ASTRID CONSTITUTION:"]
...
def _list_of_strings(raw: dict[str, Any], key: str) -> list[str]:
    ...
    if not isinstance(values, list):
        raise ValueError(f"Expected '{key}' to be a list in Astrid constitution.")
    return [str(value) for value in values]


def _relationship_modes(raw: Any) -> dict[str, dict[str, str]]:
    if not isinstance(raw, dict):
        raise ValueError("Expected 'relationship_modes' to be a mapping in Astrid constitution.")
...
def _mapping(raw: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    if not isinstance(values, dict):
        raise ValueError(f"Expected '{key}' to be a mapping in Astrid constitution.")
...
def _read_yaml(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(loaded, dict):
        raise ValueError("Expected Astrid constitution YAML to contain a mapping.")
    return loaded
