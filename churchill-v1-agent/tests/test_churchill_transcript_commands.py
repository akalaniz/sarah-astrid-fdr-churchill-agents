...
class ChurchillTranscriptCommandTests(unittest.TestCase):
    def test_transcripts_reports_historical_bus_worldline(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with _patched_transcript_paths(env):
                ...
                self.assertIn("agent: Churchill v1.0", text)
                self.assertIn("worldline: fdr_churchill_historical", text)
                self.assertIn(r"shared_fdr_churchill_bus", text)
    ...
    def test_confirmed_clear_preserves_latest_and_untouched_state_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with _patched_transcript_paths(env):
                ...
                self.assertEqual(env.sarah_bus.read_text(encoding="utf-8"), "sarah/astrid sentinel")
                ...
                self.assertEqual(env.fdr_marker.read_text(encoding="utf-8"), "fdr sentinel")
    ...
    def test_safety_refuses_wrong_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            unsafe = root / "shared_agent_bus" / "transcripts"
...
class TranscriptEnv:
    def __init__(self, root: Path) -> None:
        self.root = root
        self.shared_bus = root / "shared_fdr_churchill_bus"
        ...
        self.memory_file = root / "churchill-v1-agent" / "data" / "memory" / "churchill_memory.jsonl"
        self.sarah_bus = root / "shared_agent_bus" / "agent_messages.jsonl"
        self.sarah_marker = root / "sarah-v2-agent" / "marker.txt"
        self.astrid_marker = root / "astrid-v1-agent" / "marker.txt"
        self.fdr_marker = root / "fdr-v1-agent" / "marker.txt"
...
def _test_app(root: Path):
    ...
    memory_file = root / "churchill-v1-agent" / "data" / "memory" / "churchill_memory.jsonl"
    settings = settings.__class__(
        **{
            **settings.__dict__,
            "memory_file": memory_file,
            "conversations_dir": root / "churchill-v1-agent" / "data" / "conversations",
            "web_cache_dir": root / "churchill-v1-agent" / "data" / "web_cache",
            "source_docs_dir": root / "churchill-v1-agent" / "data" / "source_docs",
            "external_source_docs_dir": root / "churchill-v1-agent" / "data" / "source_docs",
            "vector_store_dir": root / "churchill-v1-agent" / "data" / "vector_store",
        }
    )
