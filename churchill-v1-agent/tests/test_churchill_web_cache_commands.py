...
class ChurchillWebCacheCommandTests(unittest.TestCase):
    def test_web_cache_reports_churchill_cache_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with _patched_cache_paths(env):
                ...
                self.assertIn(r"churchill-v1-agent", text)
    ...
    def test_safety_refuses_wrong_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            unsafe = root / "churchill-v1-agent" / "data" / "memory"
            unsafe.mkdir(parents=True)
            with patch.object(wc, "PROJECT_ROOT", root / "churchill-v1-agent"):
...
def _test_app(root: Path):
    ...
    project = root / "churchill-v1-agent"
    settings = settings.__class__(
        **{
            **settings.__dict__,
            "project_root": project,
            "data_dir": project / "data",
            "memory_file": project / "data" / "memory" / "churchill_memory.jsonl",
