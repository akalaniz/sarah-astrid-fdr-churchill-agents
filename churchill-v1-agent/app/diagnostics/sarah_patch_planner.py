...
PATCH_CATALOG: tuple[FDRPatchSpec, ...] = (
    FDRPatchSpec(
    ...
    FDRPatchSpec(
        "reload config/churchill_master_prompt.md every session",
        "master_prompt",
        ...
        "Load config/churchill_master_prompt.md at session startup and avoid stale cached identity text.",
        82,
    ),
    FDRPatchSpec(
        "validate config/churchill_constitution.yaml loading",
        "constitution",
    ...
    FDRPatchSpec(
        ...
        "source_docs",
        ("source_docs_not_ingested", "source_docs_not_reingested", "churchill_doc_unavailable", "vector_store_stale"),
        "medium",
        "python -m app.rag.retriever probes Churchill, Darwin, Mars, and Universe filenames",
    ...
    FDRPatchSpec(
        ...
        "Keep Alex memories and Churchill self-continuity in separate namespaces with explicit loaders.",
        86,
    ),
    FDRPatchSpec(
    ...
    FDRPatchSpec(
        ...
        "Assert CLI and web use the same Churchill engine, model config, memory path, prompt builder, and retriever.",
        92,
    ),
    FDRPatchSpec(
        "add source-doc coverage test",
        "eval_tests",
        ("source_docs_not_ingested", "churchill_doc_unavailable", "darwin_doc_not_retrieved_for_darwin_questions"),
        "low",
        "tests.test_source_doc_coverage verifies required filenames exist in vector store metadata",
        "Check required FDR source documents are present in retrieval metadata after ingestion.",
        81,
    ),
    FDRPatchSpec(
        ...
        "low",
        "app.evals.run_evals checks no Pick one/choose one/FDR-as menu phrases",
        "Add a regression case that rejects multiple-choice intimacy menus and preserves Churchill's adult voice.",
        91,
    ),
    FDRPatchSpec(
...
def _reduction_factor(spec: FDRPatchSpec) -> float:
