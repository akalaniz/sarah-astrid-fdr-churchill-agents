...
def _perrow_matrix_html(graph: nx.DiGraph) -> str:
    ...
    svg = f"""
    <svg viewBox="0 0 {width} {height}" role="img" aria-label="Churchill Perrow matrix">
      <rect width="{width}" height="{height}" fill="#fbfcff"/>
      <text x="{width / 2}" y="32" text-anchor="middle" class="title">Churchill Perrow Matrix</text>
      {grid}
    ...
    """
    return _html_page("Churchill Perrow Matrix", svg, _legend_html())
...
def _cascade_timeline_html(cascade: dict[str, Any]) -> str:
    ...
    return _html_page("Churchill Cascade Timeline", svg, legend)
...
def _patch_priority_html(patches: list[dict[str, Any]]) -> str:
    ...
    """
    return _html_page("Churchill Patch Priorities", svg, "<p>Ranked by expected risk reduction. Priority also includes centrality, known findings, cascade observations, and difficulty.</p>")
...
def _route_divergence_html(graph: nx.DiGraph) -> str:
    ...
    nodes = {
        ...
        "engine": (360, 235, "Shared Churchill engine", "#57a773"),
    ...
    """
    return _html_page("Churchill Route Divergence", svg, diff_notes)
...
def _failure_category(failure: str) -> str:
    ...
    if any(token in text for token in ("web", "route", "browser", "cli")):
        return "route"
    if any(token in text for token in ("intimacy", "churchill", "coy", "sex", "hr_safe")):
        return "intimacy"
...
def _graphml_safe_copy(graph: nx.DiGraph) -> nx.DiGraph:
    safe = nx.DiGraph(name=graph.graph.get("name", "Churchill vulnerability graph"))
    for node_id, attrs in graph.nodes(data=True):
