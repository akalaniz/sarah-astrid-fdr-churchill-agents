...
DEFAULT_CONSTITUTION_PATH = PROJECT_ROOT / "config" / "churchill_constitution.yaml"
...
def load_sarah_constitution(path: Path = DEFAULT_CONSTITUTION_PATH) -> FDRConstitution:
    if not path.exists():
        raise FileNotFoundError(f"Churchill constitution file not found: {path}")
...
def _list_of_strings(raw: dict[str, Any], key: str) -> list[str]:
    ...
    if not isinstance(values, list):
        raise ValueError(f"Expected '{key}' to be a list in Churchill constitution.")
    return [str(value) for value in values]


def _relationship_modes(raw: Any) -> dict[str, dict[str, str]]:
    if not isinstance(raw, dict):
        raise ValueError("Expected 'relationship_modes' to be a mapping in Churchill constitution.")
...
def _mapping(raw: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    if not isinstance(values, dict):
        raise ValueError(f"Expected '{key}' to be a mapping in Churchill constitution.")
...
def _read_yaml(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(loaded, dict):
        raise ValueError("Expected Churchill constitution YAML to contain a mapping.")
    return loaded
