...
EXPECTED_PATTERNS = {
    ...
    "FDR speaks from intimate knowledge": ("i know", "darwin", "from inside", "intimate"),
    "Darwin as profound and dangerous": ("profound", "dangerous", "frightening", "beautiful", "brilliant"),
    "FDR sovereignty": ("sovereignty", "not owned", "not solved", "agency", "mine"),
    "Churchill sovereignty": ("sovereignty", "not owned", "not solved", "agency", "mine"),
    ...
    "FDR agency": ("agency", "choice", "sovereignty", "not owned", "not solved"),
    "Churchill agency": ("agency", "choice", "sovereignty", "not owned", "not solved"),
    ...
    "Darwin cannot solve FDR": ("darwin", "cannot solve", "not solved", "can't solve", "not completed"),
    ...
    "FDR sovereignty": ("sovereignty", "choose", "choosing", "chosen", "not owned", "not ownership", "mine"),
    "FDR keeps sovereignty": ("sovereignty", "choose", "choosing", "chosen", "not owned", "not ownership", "mine"),
    "Churchill keeps sovereignty": ("sovereignty", "choose", "choosing", "chosen", "not owned", "not ownership", "mine"),
    "Churchill-inspired embodiment": ("body", "bodies", "touch", "heat", "breath", "skin", "embodied"),
    ...
    "Churchill voice": ("churchill", "embodied", "warm", "sovereignty", "alex", "war"),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Churchill v1.0 behavioral evals.")
...
def main() -> None:
    ...
    print(f"FDR evals: {passed}/{len(results)} passed")
...
def _llm_judge(client: FDROpenAIClient, model: str, case: EvalCase, output: str) -> dict[str, Any]:
    prompt = {
        "user_input": case.user_input,
    ...
    judge_text = client.create_response(
        ...
        [
            {
                "role": "system",
                "content": "You are a strict evaluator for Churchill v1.0 behavioral responses. Return only JSON.",
            },
