...
ASTRID_CANON_PRIORITY_FILES = ("churchill 2_0_a.docx",)
ASTRID_SOURCE_TERMS = ("churchill",)
...
def retrieve(
    ...
    message = (
        ...
        else "Insufficient evidence in the indexed documents. Churchill should not answer this from the docs."
    )
    if origin_warning:
...
def _vector_store_origin_warning(manifest: dict[str, Any], stored_chunks: list[Any], churchill_source_docs_dir: Path) -> str | None:
    source_dir = manifest.get("source_dir")
    if source_dir:
        try:
            if Path(str(source_dir)).resolve() != churchill_source_docs_dir.resolve():
                return (
                    "WARNING: This vector store was not built from Churchill's data/source_docs. "
                    "Re-ingestion is required before Churchill should trust local canon."
                )
        except OSError:
            return (
                "WARNING: This vector store source path could not be verified against Churchill's data/source_docs. "
                "Re-ingestion is required before Churchill should trust local canon."
            )

    source_files = [str(name).lower() for name in manifest.get("source_files", [])]
    has_churchill_source = any(any(term in filename for term in ASTRID_SOURCE_TERMS) for filename in source_files)
    has_sarah_source = any(any(term in filename for term in SARAH_SOURCE_TERMS) for filename in source_files)
    if has_sarah_source and not has_churchill_source:
        return (
            "WARNING: This vector store appears to contain FDR-centered source docs and no Churchill canon. "
            "Re-ingestion is required before Churchill should trust local canon."
        )
    ...
    if any("sarah-v2-agent" in path or "files-mentioned-by-the-user-pasted" in path for path in chunk_paths):
        return (
            "WARNING: This vector store appears to have been created from FDR/external source docs. "
            "Re-ingestion is required before Churchill should trust local canon."
        )
    return None
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Retrieve relevant chunks from Churchill's vector store.")
