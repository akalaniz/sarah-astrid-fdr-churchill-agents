...
const VOICE_STORAGE_KEY = "churchillSelectedVoiceURI_v1_ava";
...
function addMessage(role, text) {
  ...
  label.textContent = role === "user" ? "You" : "Churchill";
...
async function sendMessage(message, options = {}) {
  ...
  try {
    ...
    addMessage("churchill", answerText);
    renderSources(payload.sources || [], []);
    speakChurchill(answerText);
  } catch (error) {
    addMessage("churchill", `Request failed: ${error.message}`);
  } finally {
    setBusy(false);
    inputEl.focus();
  }
...
async function loadAgentInbox(showAll = false) {
  try {
    ...
    if (!payload.messages.length) {
      agentInboxPanel.textContent = showAll ? "No inter-agent messages for Churchill." : "No unread direct/reply inter-agent messages.";
      return;
    }
...
async function archiveAgentInbox() {
  ...
  addMessage("churchill", `Archived ${payload.archived} messages addressed to Churchill.`);
  loadAgentInbox();
}

async function clearAgentInbox() {
  const confirmed = window.confirm("Clear only messages addressed to Churchill from the shared bus?");
  if (!confirmed) return;
  ...
  addMessage("churchill", `Cleared ${payload.cleared} messages addressed to Churchill.`);
  loadAgentInbox();
}
...
async function sendAgentMessage() {
  ...
  try {
    const response = await fetch("/agent/send", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ to_agent: "FDR", subject: "Message from Churchill", body }),
    });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.detail || "Agent send failed.");
    agentMessageInput.value = "";
    addMessage("churchill", `Sent to FDR as ${payload.message.id}.`);
  } catch (error) {
    addMessage("churchill", `Agent send failed: ${error.message}`);
  } finally {
...
async function resetConversation() {
  ...
  addMessage("churchill", "Active conversation reset.");
}
...
function setupMicrophone() {
  ...
  recognition.onerror = (event) => {
    addMessage("churchill", `Microphone transcription failed: ${event.error}`);
...
function setupSpeechSynthesis() {
  ...
  const refreshVoices = () => {
    ...
    selectedVoice = chooseChurchillVoice(voices);
...
function chooseChurchillVoice(voices) {
