...
class AgentSendRequest(BaseModel):
    to_agent: str = "FDR"
    subject: str = "Message from Churchill"
    body: str
...
def create_app(settings: Settings | None = None, state: AppState | None = None) -> FastAPI:
    ...
    app = FastAPI(title="Churchill v1.0 Local Web UI")
    ...
    def agent_read(request: AgentReadRequest) -> dict[str, Any]:
        ...
        if not ok or message is None:
            raise HTTPException(status_code=404, detail="Message not found for Churchill.")
        return {"message": message}
...
def _handle_web_command(message: str, sarah_state: AppState) -> dict[str, Any] | None:
    ...
    if command == "/memory":
        return {
            "text": sarah_engine.format_churchill_memories(),
            "sources": [],
            "command": "memory",
        }
    ...
    if command.startswith("/memory "):
        ...
        return {
            "text": sarah_engine.format_churchill_memories(keyword),
            "sources": [],
    ...
    if command.startswith("/remember "):
        ...
        if not memory_text:
            raise HTTPException(status_code=400, detail="Memory text cannot be empty.")
        record = sarah_engine.remember_churchill_memory(memory_text, session_id=sarah_state.session_id)
        return {
    ...
    if command.startswith("/recall "):
        ...
        return {
            "text": sarah_engine.recall_churchill_memories(keyword, session_id=sarah_state.session_id),
            "sources": [],
            "command": "recall",
        }
    ...
    if command == "/recall_all":
        return {
            "text": sarah_engine.recall_all_churchill_memories(session_id=sarah_state.session_id),
            "sources": [],
            "command": "recall_all",
        }
    ...
    if command in {"/use_memory on", "/use_memory off"}:
        ...
        return {
            "text": sarah_engine.set_churchill_use_memory(enabled, session_id=sarah_state.session_id),
            "sources": [],
            "command": "use_memory",
            "use_memory": enabled,
        }
    ...
    if command == "/clear_recall":
        return {
            "text": sarah_engine.clear_churchill_recall(session_id=sarah_state.session_id),
            "sources": [],
    ...
    if command == "/debug_memory_live":
        return {
            "text": sarah_engine.format_debug_churchill_memory_live(session_id=sarah_state.session_id),
            "sources": [],
            "command": "debug_memory_live",
            "debug": sarah_engine.debug_churchill_memory_live(session_id=sarah_state.session_id),
        }
    ...
    if command == "/debug_churchill_intimacy":
        last_reply = sarah_engine.get_last_sarah_reply(sarah_state.session_id)
        debug = last_reply.safety_debug if last_reply else None
        return {
            "text": _format_debug_churchill_intimacy(debug),
            "sources": [],
            "command": "debug_churchill_intimacy",
            "debug": debug or {},
        }
    ...
    if command.startswith("/read "):
        ...
        return {"text": "Message not found for Churchill.", "sources": [], "command": "read"}
...
def _parse_send_command(message: str, command_name: str = "/send") -> tuple[str, str]:
    ...
    if ":" not in payload:
        raise HTTPException(status_code=400, detail=f'Use {command_name} FDR: message text')
...
def _handle_web_cache_command(message: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        if command == "/web_cache_clear":
            text = "This will delete all Churchill web-cache JSON files from data\\web_cache. To confirm, run: /web_cache_clear confirm"
        ...
        if command == "/web_cache_archive_all":
            text = "To archive all Churchill web-cache JSON files, run: /web_cache_archive_all confirm"
        ...
        if command == "/web_cache_delete_corrupt":
            text = "This will delete corrupt Churchill web-cache JSON files only. To confirm, run: /web_cache_delete_corrupt confirm"
...
def _run_crew_command(crew_command: Any, default_first_speaker: str) -> dict[str, Any]:
    if _requires_unavailable_fdr(crew_command.agents):
        return _fdr_unavailable_response("crew")
    try:
...
def _run_debate_alex_command(debate_command: Any) -> dict[str, Any]:
    if debate_command.judge.lower() == "fdr" and not is_agent_endpoint_reachable("FDR"):
        return _fdr_unavailable_response("debate_alex")
    try:
...
def _requires_unavailable_fdr(agents: list[str]) -> bool:
    return any(agent.lower() == "fdr" for agent in agents) and not is_agent_endpoint_reachable("FDR")
...
def _fdr_unavailable_response(command: str) -> dict[str, Any]:
    text = _fdr_not_running_message()
    return {"ok": False, "error": text, "text": text, "sources": [], "command": command}


def _format_debug_churchill_intimacy(debug: dict[str, Any] | None) -> str:
    if not debug:
        return "No Churchill intimacy debug information has been recorded yet."
    return "\n".join(
        [
            "Churchill intimacy debug:",
            ...
            "churchill_prudishness_suppressor_applied: "
            + str(debug.get("churchill_prudishness_suppressor_applied", False)).lower(),
...
<html lang="en">
  ...
  <body>
    ...
    <label>Agents<input id="agents" value="Churchill,FDR"></label>
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the Churchill v1.0 local web UI.")
    parser.add_argument("--host", default="127.0.0.1", help="Bind host. Defaults to 127.0.0.1.")
    parser.add_argument("--port", type=int, default=8011, help="Bind port. Defaults to 8011.")
    return parser
...
def main() -> None:
    ...
    if args.host != "127.0.0.1":
        print("Warning: Churchill web UI has no authentication. Prefer --host 127.0.0.1.")
