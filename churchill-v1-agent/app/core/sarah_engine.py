...
from app.core.sarah_response import FDRResponse, generate_sarah_response, response_metadata
...
_client: FDROpenAIClient | None = None
_sessions: dict[str, _FDRSession] = {}
_shared_pending_recall: tuple[str, str, list[MemoryRecord]] | None = None
_lock = threading.Lock()


def configure_sarah_engine(settings: Settings | None = None, client: FDROpenAIClient | None = None) -> None:
    """Configure shared FDR runtime dependencies for CLI, web, or tests."""
...
def generate_sarah_reply(user_message: str, session_id: str = "default") -> FDRReply:
    ...
    with _lock:
        ...
        session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
...
def get_last_sarah_reply(session_id: str = "default") -> FDRReply | None:
    with _lock:
        session = _sessions.get(session_id)
...
def remember_churchill_memory(text: str, session_id: str = "default") -> MemoryRecord:
    settings = _get_settings()
    return MemoryStore(settings.memory_file).remember(text)


def list_churchill_memories(keyword: str = "") -> list[MemoryRecord]:
    settings = _get_settings()
    store = MemoryStore(settings.memory_file)
    return store.search(keyword) if keyword.strip() else store.list_memories()


def format_churchill_memories(keyword: str = "") -> str:
    return format_memories(list_churchill_memories(keyword))


def recall_churchill_memories(keyword: str, session_id: str = "default") -> str:
    global _shared_pending_recall
    settings = _get_settings()
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    ...
    if not memories:
        return f"No Churchill memories matched '{keyword}'."
    lines = [f"Recalled Churchill memories for '{keyword}':"]
    lines.extend(f"- {memory.memory_type}: {memory.text}" for memory in memories[:8])
    return "\n".join(lines)


def recall_all_churchill_memories(session_id: str = "default") -> str:
    global _shared_pending_recall
    settings = _get_settings()
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    ...
    _shared_pending_recall = (session_id, session.last_recall_keyword, memories) if memories else None
    if not memories:
        return "No Churchill memories are stored."
    lines = ["Loaded all Churchill memories. This may make the next response less focused."]
...
def set_churchill_use_memory(enabled: bool, session_id: str = "default") -> str:
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    session.use_memory = enabled
    return f"use_memory: {'on' if enabled else 'off'}"


def clear_churchill_recall(session_id: str = "default") -> str:
    global _shared_pending_recall
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    session.pending_recall = []
    session.last_recall_keyword = ""
    if _shared_pending_recall and _shared_pending_recall[0] == session_id:
        _shared_pending_recall = None
    return "Pending recalled Churchill memory context cleared."


def debug_churchill_memory_live(session_id: str = "default") -> dict[str, Any]:
    settings = _get_settings()
    session = _sessions.setdefault(session_id, _FDRSession(conversation=ConversationMemory()))
    store = MemoryStore(settings.memory_file)
...
def format_debug_churchill_memory_live(session_id: str = "default") -> str:
    debug = debug_churchill_memory_live(session_id)
...
def _get_client(settings: Settings) -> FDROpenAIClient:
    global _client
    if _client is None:
        _client = FDROpenAIClient(settings.openai_api_key)
    return _client


def _prepare_memory_context(session: _FDRSession, session_id: str) -> tuple[str | None, int, str | None]:
...
def _mark_shared_recall_used(source_session_id: str, response: FDRResponse, injected_count: int) -> None:
    global _shared_pending_recall
...
def _prompt_had_memory(response: FDRResponse) -> bool:
    block = _extract_memory_block(response)
    return "Relevant remembered context from Churchill memory:" in block and "No relevant Churchill memories injected" not in block


def _count_injected_memories(response: FDRResponse) -> int:
    block = _extract_memory_block(response)
    return sum(1 for line in block.splitlines() if line.strip().startswith("* "))


def _extract_memory_block(response: FDRResponse) -> str:
    for message in response.messages:
        if "Relevant remembered context from Churchill memory:" in message.get("content", ""):
...
def _forbidden_memory_paths_detected(memory_file) -> bool:
    ...
    forbidden = (
        ...
        r"astrid-v1-agent\data\memory\astrid_memory.jsonl",
        r"sarah-v2-agent\data\memory\fdr_memory.jsonl",
