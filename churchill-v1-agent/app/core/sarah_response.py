...
def generate_sarah_response(
    ...
    if safety_decision.blocked_by_app:
        safety_debug = _build_churchill_intimacy_debug(user_input, safety_decision.as_dict(), [])
        return FDRResponse(
    ...
    retrieval, retrieval_error = _retrieve_for_message(user_input)
    ...
    cas_context_packet = (
        None
        if detected_mode == FDRMode.PERROW_CAS_ACCIDENT_ANALYSIS
    ...
    safety_debug = _build_churchill_intimacy_debug(user_input, safety_decision.as_dict(), assembly.messages)
    return FDRResponse(
        answer=answer,
        retrieval=retrieval,
...
def _build_perrow_cas_context_packet(user_input: str, detected_mode: FDRMode) -> str | None:
    if detected_mode != FDRMode.PERROW_CAS_ACCIDENT_ANALYSIS:
        return None
...
def response_metadata(response: FDRResponse) -> dict[str, Any]:
...
def _build_churchill_intimacy_debug(
    user_input: str,
    ...
    triggered = safety_debug.get("detected_mode") == FDRMode.ADULT_CONSENSUAL_FICTION.value
    ...
    enriched.update(
        {
            ...
            "churchill_prudishness_suppressor_applied": bool(triggered and prompt_excerpt),
...
def _adult_consensual_prompt_excerpt(messages: list[dict[str, str]]) -> str:
    marker = "You are Winston Churchill: embodied, warm, sensual"
