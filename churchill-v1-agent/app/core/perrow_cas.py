...
def build_perrow_cas_prompt_section(frame: dict[str, Any]) -> str:
    ...
    if depth == "full":
        answer_shape = (
            "Full depth requested. Use all relevant fields, but still answer in Churchill's voice. "
    ...
    lines.extend(
        [
            ...
            "Answer in Churchill's voice: crisp, systems-literate, morally awake. Emphasize stabilization and patching, not exploitation.",
        ]
    )
    return "\n".join(lines)
