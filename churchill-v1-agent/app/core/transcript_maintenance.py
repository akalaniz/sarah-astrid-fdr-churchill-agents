...
SHARED_BUS_DIR = PROJECT_ROOT.parent / "shared_fdr_churchill_bus"
...
def get_churchill_transcript_paths() -> tuple[Path, Path]:
...
def transcript_status(transcripts_path: Path | None = None, archive_path: Path | None = None) -> dict[str, Any]:
    ...
    if error:
        return _with_error({"agent": "Churchill v1.0", "worldline": "fdr_churchill_historical"}, error)
...
def keep_newest_conversations(transcripts_path: Path | None = None, keep_n: int = 20) -> dict[str, Any]:
    ...
    error = _safety_error(transcripts_path, archive_path)
    if error:
        return _with_error(_keep_result(transcripts_path), error)
...
def _safety_error(transcripts_path: Path, archive_path: Path) -> str:
    ...
    if transcripts.name != "transcripts" or transcripts.parent.name != "shared_fdr_churchill_bus":
        return f"Refusing transcript cleanup: unsafe transcript path detected: {transcripts_path}"
    if archive.name != "transcripts_archive" or archive.parent.name != "shared_fdr_churchill_bus":
...
def _keep_result(transcripts_path: Path) -> dict[str, Any]:
    return {
