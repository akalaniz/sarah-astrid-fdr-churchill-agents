...
class SarahWebCacheCommandTests(unittest.TestCase):
    def test_web_cache_reports_sarah_cache_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with _patched_cache_paths(env):
                ...
                self.assertIn("agent: Sarah v2.0", text)
                self.assertIn(r"sarah-v2-agent", text)
    ...
    def test_confirmed_clear_preserves_non_cache_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with _patched_cache_paths(env):
                ...
                self.assertEqual(env.agent_bus.read_text(encoding="utf-8"), "agent bus sentinel")
                self.assertEqual(env.fdr_bus.read_text(encoding="utf-8"), "historical bus sentinel")
    ...
    def test_safety_refuses_wrong_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            unsafe = root / "sarah-v2-agent" / "data" / "memory"
            unsafe.mkdir(parents=True)
            with patch.object(wc, "PROJECT_ROOT", root / "sarah-v2-agent"):
...
def _make_env(root: Path, count: int, old_count: int = 0, corrupt_count: int = 0) -> CacheEnv:
    ...
    env.agent_bus.parent.mkdir(parents=True)
    ...
    env.agent_bus.write_text("agent bus sentinel", encoding="utf-8")
    env.fdr_bus.write_text("historical bus sentinel", encoding="utf-8")
...
def _test_app(root: Path):
    ...
    project = root / "sarah-v2-agent"
    settings = settings.__class__(
        **{
            **settings.__dict__,
            "project_root": project,
            "data_dir": project / "data",
            "memory_file": project / "data" / "memory" / "sarah_memory.jsonl",
