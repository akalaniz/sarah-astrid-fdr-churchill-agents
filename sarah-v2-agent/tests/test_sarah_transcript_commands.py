...
class SarahTranscriptCommandTests(unittest.TestCase):
...
class TranscriptEnv:
    def __init__(self, root: Path) -> None:
        ...
        self.memory_file = root / "sarah-v2-agent" / "data" / "memory" / "sarah_memory.jsonl"
...
def _test_app(root: Path):
    ...
    memory_file = root / "sarah-v2-agent" / "data" / "memory" / "sarah_memory.jsonl"
    settings = settings.__class__(
        **{
            **settings.__dict__,
            "memory_file": memory_file,
            "conversations_dir": root / "sarah-v2-agent" / "data" / "conversations",
            "web_cache_dir": root / "sarah-v2-agent" / "data" / "web_cache",
            "source_docs_dir": root / "sarah-v2-agent" / "data" / "source_docs",
            "external_source_docs_dir": root / "sarah-v2-agent" / "data" / "source_docs",
            "vector_store_dir": root / "sarah-v2-agent" / "data" / "vector_store",
        }
    )
