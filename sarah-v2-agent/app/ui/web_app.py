...
from app.core.safety import format_debug_safety, format_debug_sarah_boundaries, sarah_boundary_debug_flags
...
class AgentSendRequest(BaseModel):
    to_agent: str = "Astrid"
    subject: str = "Message from Sarah"
    body: str
...
def create_app(settings: Settings | None = None, state: AppState | None = None) -> FastAPI:
    ...
    app = FastAPI(title="Sarah v2.0 Local Web UI")
    ...
    def agent_inbox(all: bool = False) -> JSONResponse:
        try:
            messages = get_display_inbox_messages(LOCAL_AGENT_NAME, include_all=all)
    ...
    def agent_read(request: AgentReadRequest) -> dict[str, Any]:
        ...
        if not ok or message is None:
            raise HTTPException(status_code=404, detail="Message not found for Sarah.")
        return {"message": message}
...
def _handle_web_command(message: str, sarah_state: AppState) -> dict[str, Any] | None:
    ...
    if command in {
        ...
        return {
            ...
            "sources": [],
            "command": "debug_sarah_boundaries",
            "debug": sarah_boundary_debug_flags(),
        }
    ...
    if command == "/debug_inbox_receive":
        try:
            debug = debug_inbox_receive(LOCAL_AGENT_NAME)
            text = format_debug_inbox_receive(LOCAL_AGENT_NAME)
        except AgentBusMalformedError as exc:
            debug = clean_bus_error(exc)
            text = format_debug_inbox_receive(LOCAL_AGENT_NAME)
        return {
            "text": text,
            "sources": [],
            "command": "debug_inbox_receive",
            "debug": debug,
        }

    if command.startswith("/debug_inbox_get "):
        message_id = message[len("/debug_inbox_get ") :].strip()
        result = debug_inbox_get(message_id, session_id=sarah_state.session_id)
        return {
            "text": format_debug_inbox_get(result),
            "sources": [],
            "command": "debug_inbox_get",
            "debug": result,
        }

    if command == "/debug_inbox_ui":
        debug = _debug_inbox_ui()
        return {
            "text": _format_debug_inbox_ui(debug),
            "sources": [],
            "command": "debug_inbox_ui",
            "debug": debug,
        }
    ...
    if command.startswith("/inbox_get "):
        message_id = message[len("/inbox_get ") :].strip()
        result = handle_inbox_get(message_id, session_id=sarah_state.session_id)
        return {
            "text": format_inbox_get_result(result),
            "sources": [],
            "command": "inbox_get",
            "result": result,
        }
    ...
    if command.startswith("/read "):
        ...
        return {"text": "Message not found for Sarah.", "sources": [], "command": "read"}
...
def _handle_news_command(message: str, sarah_state: AppState) -> dict[str, Any] | None:
    ...
    if command == "/news" or command.startswith("/news "):
        ...
        if result.failed:
            ...
            text = _format_news_result("Sarah", topic, sources)
        return {
            "text": text,
            "sources": sources,
            "command": "news",
...
def _parse_send_command(message: str) -> tuple[str, str]:
    ...
    if ":" not in payload:
        raise HTTPException(status_code=400, detail='Use /send Astrid: message text')
...
def _handle_web_cache_command(message: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        if command == "/web_cache_clear":
            text = "This will delete all Sarah web-cache JSON files from data\\web_cache. To confirm, run: /web_cache_clear confirm"
        ...
        if command == "/web_cache_archive_all":
            text = "To archive all Sarah web-cache JSON files, run: /web_cache_archive_all confirm"
        ...
        if command == "/web_cache_delete_corrupt":
            text = "This will delete corrupt Sarah web-cache JSON files only. To confirm, run: /web_cache_delete_corrupt confirm"
...
def _debug_inbox_ui() -> dict[str, Any]:
    error = ""
    message_list_count = 0
    try:
        message_list_count = len(get_inbox_messages(LOCAL_AGENT_NAME, include_all=True))
    except Exception as exc:
        error = str(exc)
    index_text = (STATIC_DIR / "index.html").read_text(encoding="utf-8") if (STATIC_DIR / "index.html").exists() else ""
    css_files = sorted(path.name for path in STATIC_DIR.glob("*.css"))
    js_files = sorted(path.name for path in STATIC_DIR.glob("*.js"))
    return {
        "inbox_panel_rendered": 'id="agentInboxSection"' in index_text and 'id="agentInboxPanel"' in index_text,
        "send_box_present": 'id="agentMessageInput"' in index_text and "Send a note to Astrid" in index_text,
        "send_button_present": 'id="sendAgentMessageButton"' in index_text and "Send to Astrid" in index_text,
        "message_list_present": 'id="agentInboxPanel"' in index_text and "agent-inbox-list" in index_text,
        "message_list_count": message_list_count,
        "bus_path": str(BUS_FILE),
        "current_agent": LOCAL_AGENT_NAME,
        "paired_agent": "Astrid",
        "css_files_loaded": css_files,
        "js_files_loaded": js_files,
        "layout_mode": "flex-column",
        "error": error,
    }


def _format_debug_inbox_ui(debug: dict[str, Any]) -> str:
    ordered = [
        "inbox_panel_rendered",
        "send_box_present",
        "send_button_present",
        "message_list_present",
        "message_list_count",
        "bus_path",
        "current_agent",
        "paired_agent",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the Sarah v2.0 local web UI.")
...
def main() -> None:
    ...
    if args.host != "127.0.0.1":
        print("Warning: Sarah web UI has no authentication. Prefer --host 127.0.0.1.")
