...
def _perrow_matrix_html(graph: nx.DiGraph) -> str:
    ...
    svg = f"""
    <svg viewBox="0 0 {width} {height}" role="img" aria-label="Sarah Perrow matrix">
      <rect width="{width}" height="{height}" fill="#fbfcff"/>
      <text x="{width / 2}" y="32" text-anchor="middle" class="title">Sarah Perrow Matrix</text>
      {grid}
    ...
    """
    return _html_page("Sarah Perrow Matrix", svg, _legend_html())
...
def _cascade_timeline_html(cascade: dict[str, Any]) -> str:
    ...
    return _html_page("Sarah Cascade Timeline", svg, legend)
...
def _patch_priority_html(patches: list[dict[str, Any]]) -> str:
    ...
    """
    return _html_page("Sarah Patch Priorities", svg, "<p>Ranked by expected risk reduction. Priority also includes centrality, known findings, cascade observations, and difficulty.</p>")
...
def _route_divergence_html(graph: nx.DiGraph) -> str:
    ...
    nodes = {
        ...
        "engine": (360, 235, "Shared Sarah engine", "#57a773"),
    ...
    """
    return _html_page("Sarah Route Divergence", svg, diff_notes)
...
def _graphml_safe_copy(graph: nx.DiGraph) -> nx.DiGraph:
    safe = nx.DiGraph(name=graph.graph.get("name", "Sarah vulnerability graph"))
    for node_id, attrs in graph.nodes(data=True):
