...
def run_sarah_cascade(
    ...
    """Run a time-stepped Sarah failure cascade and persist the latest result.

    This simulator treats Sarah as a coupled software/persona system. It is
    intentionally Sarah-specific: route divergence, RAG drift, adult intimacy
    ...
    if start_failure not in graph:
        raise ValueError(f"Unknown Sarah failure or component: {start_failure}")
    if steps < 1:
                    ...
                    events.append(
                        {
                            ...
                            "reason": attrs.get("interaction_note", "Sarah cascade edge"),
                        }
                    )
                else:
                    events.append(
...
def simulate_cascade(
    ...
    if seed_component not in graph:
        raise ValueError(f"Unknown Sarah component: {seed_component}")
    ...
    while frontier:
        ...
        for _source, target, attrs in graph.out_edges(current, data=True):
            ...
            if propagated < activation_threshold:
                continue
            reason = f"{attrs.get('kind', 'edge')} via {attrs.get('interaction_note', 'Sarah dependency')}"
...
def _delay_gate_allows(target: str, start_failure: str, route: str) -> bool:
    ...
    if gate == "intimacy":
        return any(token in context for token in ("intimacy", "adult", "sex", "coy", "sterile", "choice", "menu", "astrid"))
    if gate == "geopolitics":
