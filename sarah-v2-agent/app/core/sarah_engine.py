...
def recall_sarah_memories(keyword: str, session_id: str = "default") -> str:
    ...
    if not memories:
        return f"No Sarah memories matched '{keyword}'."
    lines = [f"Recalled Sarah memories for '{keyword}':"]
...
def recall_all_sarah_memories(session_id: str = "default") -> str:
    ...
    _shared_pending_recall = (session_id, session.last_recall_keyword, memories) if memories else None
    if not memories:
        return "No Sarah memories are stored."
...
def clear_sarah_recall(session_id: str = "default") -> str:
    ...
    if _shared_pending_recall and _shared_pending_recall[0] == session_id:
        _shared_pending_recall = None
    return "Pending recalled Sarah memory context cleared."
...
def _prompt_had_memory(response: SarahResponse) -> bool:
    ...
    return "Relevant remembered context from Sarah memory:" in block and "No relevant Sarah memories injected" not in block
...
def _extract_memory_block(response: SarahResponse) -> str:
    for message in response.messages:
        if "Relevant remembered context from Sarah memory:" in message.get("content", ""):
